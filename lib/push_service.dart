...
class PushService {
  ...
  static Future<void> _onMessage(RemoteMessage message) async {
    ...
    try {
      switch (command) {
        case 'familyRequest':
          if (Platform.isAndroid && message.data['deviceId'] == Preferences.instance.getString(Preferences.id)) {
            await FamilyAccess.receive(message.data);
          }
        case 'positionSingle':
...
Future<void> pushServiceBackgroundHandler(RemoteMessage message) async {
  ...
  if (message.data['command'] == 'familyRequest') {
