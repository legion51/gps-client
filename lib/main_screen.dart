...
import 'package:family_access/family_access.dart';
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (Platform.isAndroid)
            IconButton(
              tooltip: 'Семейный доступ',
              icon: const Icon(Icons.family_restroom),
              onPressed: () async {
                try {
                  await FamilyAccess.configure(Preferences.instance.getString(Preferences.id) ?? '');
                } on PlatformException {
                  messengerKey.currentState?.showSnackBar(
                    const SnackBar(content: Text('Не удалось открыть настройки семейного доступа')),
                  );
                }
              },
            ),
